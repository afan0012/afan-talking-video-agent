...
文档中出现的 `engines/musetalk-1.5` 是相对于“当前数据目录”的内部目录名，不是固定的 D 盘绝对路径。源码运行时数据目录默认为项目目录；安装版默认为 `%LOCALAPPDATA%\\afan Talking Head Agent`；用户也可以在「偏好设置」改到其他磁盘。普通用户不需要手动填写或记住完整路径。
