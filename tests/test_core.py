...
from app.digital_human import HeyGemProvider, MuseTalkProvider, _command_tokens, provider_infos
...
def test_local_digital_human_provider_contract_reports_configuration():
    infos = {item.id: item for item in provider_infos({
        "MUSETALK_COMMAND": "[\"python\",\"inference.py\"]",
        "HEYGEM_BASE_URL": "http://127.0.0.1:8383",
    })}
    assert infos["musetalk-1.5"].configured is True
    assert infos["heygem"].configured is True
    assert MuseTalkProvider({"MUSETALK_COMMAND": "[\"python\",\"x.py\"]"}).info().kind == "command"
    assert HeyGemProvider({"HEYGEM_BASE_URL": "http://127.0.0.1:8383"}).info().kind == "http"


def test_heygem_media_root_maps_shared_container_path(tmp_path):
    provider = HeyGemProvider({"HEYGEM_MEDIA_ROOT": "/workspace/media"})
    assert provider._media_ref(tmp_path / "person.mp4") == "/workspace/media/person.mp4"


def test_heygem_submission_matches_documented_task_code_contract(tmp_path):
    provider = HeyGemProvider({"HEYGEM_MEDIA_ROOT": "/workspace/media"})
    payload = provider._submission_payload(
        video=tmp_path / "person.mp4",
        audio=tmp_path / "speech.wav",
        task_code="job-123",
    )
    assert payload == {
        "audio_url": "/workspace/media/speech.wav",
        "video_url": "/workspace/media/person.mp4",
        "code": "job-123",
        "chaofen": 0,
        "watermark_switch": 0,
        "pn": 1,
...
def test_musetalk_command_template_supports_json_tokens():
    assert _command_tokens('["python", "-m", "scripts.inference", "--version", "v15"]') == [
        "python", "-m", "scripts.inference", "--version", "v15"
    ]


def test_musetalk_writes_the_official_top_level_task_mapping(tmp_path, monkeypatch):
    video = tmp_path / "person.mp4"
    audio = tmp_path / "speech.wav"
    ...
    def fake_popen(tokens, **kwargs):
        captured["tokens"] = tokens
        captured["kwargs"] = kwargs
        return CompletedProcess()

    monkeypatch.setattr(digital_human.subprocess, "Popen", fake_popen)
    provider = MuseTalkProvider({
        "MUSETALK_COMMAND": '["musetalk","--inference_config","{config}"]',
        "MUSETALK_WORKDIR": str(tmp_path),
    })
    assert provider.generate(video, audio, output) == output
    config_arg = captured["tokens"][2]
    config = json.loads(open(config_arg, encoding="utf-8").read())
    assert config == {
        "task-1": {
            "video_path": str(video),
            "audio_path": str(audio),
            "result_name": str(output),
        }
    }


def test_local_lipsync_routes_are_selectable_without_cloud_credentials(monkeypatch):
    monkeypatch.setattr(main, "_LOCAL_SETTINGS", {})
    assert main.selected_model("lipsync", "musetalk-1.5") == "musetalk-1.5"
    assert main.selected_model("lipsync", "heygem") == "heygem"
...
def test_broll_filter_supports_picture_in_picture():
    """画中画保持主画面尺寸，并按位置/比例叠加素材。"""
    filter_graph = main.build_broll_filter(
        [(1, 2.0, 3.0, "pip", "top-right", 0.25, 16)],
        "null",
        main_w=720,
        main_h=1254,
    )

    assert "scale=180:-2:force_original_aspect_ratio=decrease" in filter_graph
    assert "overlay=W-w-16:16:enable='between(t,2.000,5.000)'" in filter_graph
    assert "pad=720:1254" not in filter_graph
    assert filter_graph.rstrip().endswith("[video]")


def test_broll_filter_can_overlay_a_picture_in_picture_layout():
    """主体画中画完成后，B-roll 仍应按时间窗覆盖该布局。"""
    filter_graph = main.build_broll_filter(
        [(1, 1.0, 2.0)], "null", main_w=720, main_h=1254, base_stream="layout"
    )

    assert "[layout][broll0]overlay=0:0:enable='between(t,1.000,3.000)'" in filter_graph
    assert filter_graph.rstrip().endswith("[video]")


def test_layout_filter_supports_blurred_picture_in_picture_background():
    """主体视频画中画使用自身模糊副本做背景。"""
    filter_graph = main.build_layout_filter(
        background_type="blur",
        foreground_scale=0.72,
        foreground_position="center",
        foreground_margin=24,
    ...
    assert "boxblur=luma_radius=18" in filter_graph
    assert "scale=518:-2" in filter_graph
    assert "[layoutbg][layoutfg]overlay=(W-w)/2:(H-h)/2" in filter_graph
    assert filter_graph.endswith("[layout]")


def test_layout_filter_supports_solid_color_background():
    filter_graph = main.build_layout_filter(
        background_type="color",
        foreground_scale=0.5,
        foreground_position="bottom-right",
        foreground_margin=32,
    ...
    assert "color=c=0x123456" in filter_graph
    assert "overlay=W-w-32:H-h-32" in filter_graph
...
def test_title_and_subtitle_templates_share_one_ass_script():
    script = main.to_ass(
        [{"start": 0, "end": 2, "text": "下方字幕 关键词"}],
        title="上方标题第二行",
        title_style="classic-yellow",
        title_position="top",
        keywords="关键词",
        duration=2,
    )
    assert "Style: Title" in script
    assert "Dialogue: 1,0:00:00.00,0:00:02.00,Title" in script
    assert r"{\an8\pos(960,70)}" in script
    assert "&H00FFFF00&" in script
    assert r"\fad(120,0)" in script


def test_model_assignment_keeps_unavailable_catalogue_visible_but_disabled(tmp_path, monkeypatch):
    ...
    # 本地引擎检测直接读全局设置字典（如用户已保存 QWEN_TTS_ENGINE_ROOT），
    # 测试必须一并隔离，否则结果依赖运行机器的 .env。
    monkeypatch.setattr(main, "_LOCAL_SETTINGS", {})

    unavailable = {module["id"]: module for module in main.get_settings()["modules"]}
    assert unavailable["asr"]["models"] == []
    local_funasr = next(item for item in unavailable["asr"]["options"] if item["value"] == "local:funasr-paraformer")
    assert local_funasr["disabled"] is True
    assert "未安装" in local_funasr["availability_note"]
    assert unavailable["direct_tts"]["models"] == []
    ...
    usable = lambda step: [item["value"] for item in available[step] if item.get("available", True)]
    assert usable("asr") == ["auto", "qwen-audio-3.0-asr-flash-filetrans"]
    assert usable("subtitle_asr") == ["auto", "paraformer-realtime-v2", "qwen-audio-3.0-asr-flash-filetrans"]
    assert usable("direct_tts") == ["qwen-builtin-tts"]
    assert "mimo-v2.5" not in usable("script")
...
def test_project_intermediate_downloads_are_allowlisted(tmp_path, monkeypatch):
    from fastapi.testclient import TestClient

    monkeypatch.setattr(main, "JOBS_DIR", tmp_path)
    store = main.JobStore()
    monkeypatch.setattr(main, "store", store)
    job = make_job("download-test")
    job.preview_audio_name = "preview.wav"
    job.output_name = "result.mp4"
    store.add(job)
