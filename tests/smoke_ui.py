...
with sync_playwright() as playwright:
    ...
    assert page.locator("#qwen-controls").count() == 1
    assert page.locator("#cosy-controls").count() == 1
    ...
    assert page.locator("#broll").count() == 1
    assert page.locator("#broll-clips-list").count() == 1
    ...
    assert settings.locator('[data-settings-tab]').evaluate_all("els => els.map(el => el.dataset.settingsTab)") == ["accounts", "engines", "services", "ui"]
    settings.get_by_role("button", name="模型分配", exact=True).click()
    assert settings.locator("#settings-services").is_visible()
    assert settings.locator("select[data-route]").count() == 8
    settings.get_by_role("button", name="云端模型", exact=True).click()
    settings.get_by_role("button", name="＋ 添加供应商", exact=True).click()
    assert settings.locator("#pf-name").is_visible()
    settings.locator("#pf-name").fill("测试服务")
    settings.locator("#pf-url").fill("https://api.example.com/v1")
    settings.get_by_role("button", name="取消", exact=True).click()
