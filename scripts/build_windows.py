...
def main() -> None:
    ...
    parser.add_argument("--ffmpeg", type=Path, help="path to a redistributable ffmpeg.exe build to bundle (must include its license text)")
    ...
    command = [
        ...
        # These adapters are run by separately installed GPU engine Python
        # environments.  They are resources, not imports of the small
        # desktop process, so include them explicitly.
        "--add-data", f"{ROOT / 'scripts' / 'musetalk_remote_server.py'};scripts",
        "--add-data", f"{ROOT / 'scripts' / 'qwen_tts_server.py'};scripts",
