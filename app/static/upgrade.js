...
(function () {
  ...
  function init() {
    ...
    /* 1) 模块入口：创作 / 素材库。素材库是可用的独立入口，不要求先建项目。 */
    if (!drawer.querySelector('.module-nav')) {
      var nav = document.createElement('div');
      nav.className = 'module-nav';
      nav.innerHTML =
        '<button type="button" class="active">创作</button>' +
        '<button type="button" data-module-library title="打开本机素材库">素材库</button>';
      drawer.insertBefore(nav, drawer.firstChild);
      nav.querySelector('[data-module-library]').addEventListener('click', function () {
        var libraryButton = document.getElementById('library-btn');
